...
Also, no `catch` is required, since the `FileNotFoundException` is thrown by the method. However, you can still add a catch blok like is done here, to get information if an exception was thrown anyways.
